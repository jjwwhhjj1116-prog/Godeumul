...
## YouTube Uploader OAuth app

이 저장소의 YouTube Uploader는 채널 소유자가 승인한 영상·썸네일을 업로드하고
예약 공개 상태를 검증하는 개인용 도구다. OAuth 앱 안내와 데이터 취급 원칙은
[앱 안내](./OAUTH_APP.md), [개인정보처리방침](./PRIVACY.md),
[서비스 약관](./TERMS.md)에서 확인할 수 있다.
...
| 대본 | 권장 1,100~1,400자, 승인 예외 1,700자 이하·실측 TTS 180초 미만 | 자료 밀도에 따라 8~12분 |
...
| **2b** | 프롬프트 생성 | 실측 길이 | 컷별 6대 항목 + 실제 첫 프레임 앵커 + `visual_states` 2~4개 + 옴니 규격 | [02](./02.시각화+영상화프롬프트(고대유물).txt) | `speed_table.py` → `prompt_check.py` |
...
- 본편은 실물·도면과 고증 카드로 잠근, 축소모형임이 읽히는 박물관급 3D 디오라마
  이미지를 먼저 승인하고
  식별 유물·유적은 실사·공식 도면 형태 잠금 `I2V_LOCKED`로 생성한다. 주 유물이 없는
  발견·전쟁·노동·이동·환경 맥락은 `T2V_CONTEXT`를 허용한다. 자세한 분기는
  `02H.I2V_T2V_하이브리드_규칙.md`를 따른다.
- 모든 신규 장면은 `camera_path`에 실제 첫 프레임에서 확인한 진입점·한 진행축·한 화면
  규모·경유 경로·도착점·마지막 구도·속도 곡선·운용 방식·깊이 전환·패턴 인터럽트·
  정착점을 잠근다. 4/6/8/10초는 각각 2/3/3/4개의 `visual_states`를 프롬프트보다 먼저
  확정한다. 발견·문 개방·단면 진입은 4~6초로 나누고 사람이 따라가는 다큐멘터리
  핸드헬드/짐벌 추적으로 공간 관계를 설명한다.
- 경로·바람·물질·힘·치수·스캔·단면·분해는 장면에 맞는 형태·재질·색을 골라
  `VEO_INTEGRATED_3D`로 영상 안에 생성한다. 빨강은 위험 의미가 있을 때만 쓴다.
- 정확한 숫자·지명·증거 배지만 기존 자막·문자 렌더에서 넣는다.
...
├─ 기획/주제선정_지침.md   ★ 조사 전에 주제 각도를 검증하는 게이트
├─ 기획/주제100선.md       구버전 공학 중심 목록 — 신규 사용 금지
...
│
├─ motion-graphics/       감사용 프로토타입 — 배포 파이프라인에서 사용 금지
│
└─ 산출물/EP01_진시황릉/
    ├─ 00.팩트체크.md      근거 등급 A/B/C. C가 있으면 설명란 고지 필수
    ├─ 01.대본.txt
    ├─ 01.문맥검수.json    ★ 현재 대본 SHA + v3 한국어·문맥 전수 검문 PASS
    ├─ 02.시각화.txt
    ├─ 02a.장면구분.json / .md
    ├─ 02c.이미지선택.md
    ├─ 06.메타.json        업로더가 읽는다
...
| 발화속도 실측 | **9.35자/초** (기획은 8.0자/초·1,400자 권장, 승인 예외는 실제 TTS 180초 미만으로 판정) |
